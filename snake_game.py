...
class Food:
    def __init__(self):
        x = random.randint(0, (GAME_WID/SPACE_SIZE)-1)* SPACE_SIZE
        y = random.randint(0, (GAME_HEI/SPACE_SIZE)-1)* SPACE_SIZE
    
        self.coord = [x, y]
        canvas.create_oval(x, y, x+SPACE_SIZE, y+SPACE_SIZE, fill=FOOD_COL, tag="food")
...
def check_col(snake):
    ...
    if x<0 or x>=GAME_WID:
        return True
    elif y<0 or y>=GAME_HEI:
        return True
...
canvas = Canvas(window, bg=BG_COL, height=GAME_HEI, width=GAME_WID)
canvas.pack()
